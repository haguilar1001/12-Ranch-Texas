...
const ALIMENTOS: PlanAlimento[] = [
  ...
  // La libra del cuadro es de 0,4536 kg (libra imperial). En Colombia la libra son
  // 500 g: si fuera esa, la conejina costaría $53.550/mes y no $59.029. Por confirmar.
  { nombre: "Conejina", tipo: "concentrado", unidad: "libra", costo: 1_785, equivalencia: 454, nota: "libra de 0,4536 kg según el cuadro" },
...
const RACIONES: PlanRacion[] = [
  ...
  { bloque: "granja", alimento: "Leche 16", cantidad: 6, unidad: "kg", animal: "Cabras", categoria: null, modo: "grupal", frecuencia: "diaria", obs: "24 caprinos · 180 kg/mes = 4,5 bultos" },
...
async function main() {
  ...
  // --------------------------------------------------------------- 2) equinos
  console.log("\n" + "─".repeat(100));
  console.log("2) CENSO DE EQUINOS — 60 cabezas que no estaban en la base\n");
  ...
  for (const a of animalesBD) {
    if (!idAnimal.has(a.nombre)) idAnimal.set(a.nombre, a.id);
    if (!censo.has(a.nombre)) censo.set(a.nombre, a.cantidad);
  }
  const buscarAnimal = (nombre: string) => {
    const arriba = nombre.toLocaleUpperCase("es-CO");
    for (const [k, v] of idAnimal) if (k.toLocaleUpperCase("es-CO") === arriba) return v;
    return null;
  };
  const cabezasDe = (nombre: string) => {
    const arriba = nombre.toLocaleUpperCase("es-CO");
    for (const [k, v] of censo) if (k.toLocaleUpperCase("es-CO") === arriba) return v;
    return 0;
  };
